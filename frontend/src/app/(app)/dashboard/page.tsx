...
export default function DashboardPage() {
  ...
  const metricHelp = useMemo<Record<string, string>>(
    () => ({
      "Total Evaluations":
        "Total number of policy evaluations recorded by the backend.",
      "Approval Rate": "Approved evaluations divided by total evaluations.",
      Denials: "Number of evaluations that resulted in DENY.",
      "Avg Latency": "Average backend evaluation time in milliseconds.",
    }),
    [],
  );
  ...
  const metrics = useMemo<Metric[]>(() => {
    ...
    const approved = evaluations.filter(
      (e) => e.decision?.toUpperCase() === "APPROVE",
    ).length;
    const denied = evaluations.filter(
      (e) => e.decision?.toUpperCase() === "DENY",
    ).length;
    const approvalRate = total === 0 ? 0 : approved / total;
    const avgLatency =
      total === 0
        ? 0
        : evaluations.reduce((acc, e) => acc + (e.latencyMs ?? 0), 0) / total;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Overview of recent policy evaluations.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {metrics.map((m) => (
          <Card key={m.label}>
            <TooltipUI content={metricHelp[m.label] ?? ""} side="top">
            ...
            {m.subtext ? (
              <div className="mt-1 text-xs text-slate-500">{m.subtext}</div>
            ) : null}
          </Card>
        ))}
      </div>
