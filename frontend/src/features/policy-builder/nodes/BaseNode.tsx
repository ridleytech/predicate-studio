...
export default function BaseNode({ title, subtitle, children, tone = 'neutral' }: Props) {
  return (
    <div className={[
      ...
      {subtitle ? <div className="mt-0.5 text-sm font-medium text-slate-100">{subtitle}</div> : null}
      {children ? <div className="mt-2 text-xs text-slate-300">{children}</div> : null}
      <Handle type="source" position={Position.Right} className="!h-2 !w-2 !bg-slate-200" />
    </div>
  )
}
